...
bool Game_Interpreter::ExecuteCommand(lcf::rpg::EventCommand const& com) {
	switch (static_cast<Cmd>(com.code)) {
		...
		case 2052://Cmd:: EasyRpg_InterpolateVariable
...
double cubicBezier(double t, double p0, double p1, double p2, double p3) { //this is broken
	double u = 1 - t;
	double tt = t * t;
	double uu = u * u;
	double uuu = uu * u;
...
double getEasedT(const std::string& easingType, double t, double b, double c, double d) {
	...
	else if (easingType == "quadraticIn") {
		t /= d;
		return c * t * t + b;
	}
	else if (easingType == "quadraticOut") {
		t /= d;
		return -c * t * (t - 2) + b;
	}
	else if (easingType == "quadraticInOut") {
		t /= d / 2;
		if (t < 1) {
			return c / 2 * t * t + b;
		}
	...
	else if (easingType == "sinusoidalIn") {
		return -c * cos(t / d * (M_PI / 2)) + c + b;
	}
	else if (easingType == "sinusoidalOut") {
		return c * sin(t / d * (M_PI / 2)) + b;
	}
	else if (easingType == "sinusoidalInOut") {
		return -c / 2 * (cos(M_PI * t / d) - 1) + b;
	}
	else if (easingType == "exponentialIn") {
		return c * pow(2, 10 * (t / d - 1)) + b;
	}
	else if (easingType == "exponentialOut") {
		return c * (-pow(2, -10 * t / d) + 1) + b;
	}
	else if (easingType == "exponentialInOut") {
		t /= d / 2;
		if (t < 1) {
	...
	else if (easingType == "circularIn") {
		t /= d;
		return -c * (sqrt(1 - t * t) - 1) + b;
	}
	else if (easingType == "circularOut") {
		t = (t / d) - 1;
		return c * sqrt(1 - t * t) + b;
	}
	else if (easingType == "circularInOut") {
		t /= d / 2;
		if (t < 1) {
...
bool Game_Interpreter::CommandAnimateVariable(lcf::rpg::EventCommand const& com) {
	...
	Output::Debug("{}",list[frame->current_command].code == int(Cmd::ShowChoiceEnd));
